...
- (void)session:(MCSession *)session didReceiveString:(NSString *)str fromPeer:(MCPeerID *)peer {
    ...
    if ([str isEqualToString:@"undo"]) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"溫馨提示" message:@"對方想悔棋，是否同意" delegate:self cancelButtonTitle:@"沒門" otherButtonTitles:@"同意", nil];
        [alert show];
...
- (void)whoWinWith:(NSNotification *)notify {
    ...
    switch (type) {
        case OccupyTypeUser:
        {
            SFDraggableDialogView *v = [SFDraggableDialogView drgViewWith:self titleText:@"你贏了" photo:[UIImage imageNamed:@"winPic.jpg"] message:@"路還長，別驕傲"];
            v.delegate = self;
        ...
        case OccupyTypeAI:
        {
            SFDraggableDialogView *v = [SFDraggableDialogView drgViewWith:self titleText:@"你輸了" photo:[UIImage imageNamed:@"lostPic.jpg"] message:@"哪裡跌倒哪裡爬起來"];
            v.delegate = self;
            [self.view addSubview:v];
        ...
        {
            SFDraggableDialogView *v = [SFDraggableDialogView drgViewWith:self titleText:@"白棋贏了" photo:[UIImage imageNamed:@"winPic.jpg"] message:@"路還長，別驕傲"];
            v.delegate = self;
            [self.view addSubview:v];
        }
            break;
        ...
        {
            SFDraggableDialogView *v = [SFDraggableDialogView drgViewWith:self titleText:@"黑棋贏了" photo:[UIImage imageNamed:@"winPic.jpg"] message:@"路還長，別驕傲"];
            v.delegate = self;
            [self.view addSubview:v];
        }
            break;
