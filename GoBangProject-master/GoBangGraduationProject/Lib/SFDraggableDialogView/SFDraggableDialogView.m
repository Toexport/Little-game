...
+ (instancetype)drgViewWith:(UIViewController *)owner titleText:(NSString *)titleText photo:(UIImage *)photo message:(NSString *)messageText {
    
    SFDraggableDialogView *dialogView = [[[NSBundle mainBundle] loadNibNamed:@"SFDraggableDialogView" owner:owner options:nil] firstObject];
    dialogView.frame = [UIScreen mainScreen].bounds;
    dialogView.photo = photo;
    dialogView.titleText = [[NSMutableAttributedString alloc] initWithString:titleText];
    dialogView.messageText = [SFDraggableDialogView exampleAttributeString:messageText];
    dialogView.firstBtnText = [@"重新开始" uppercaseString];
    ...
    [dialogView createBlurBackgroundWithImage:[SFDraggableDialogView jt_imageWithView:owner.view] tintColor:[[UIColor blackColor] colorWithAlphaComponent:0.35] blurRadius:60.0];
    return dialogView;
}
...
+ (NSMutableAttributedString *)exampleAttributeString:(NSString *)messageText {

    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:messageText];
    [attributedString addAttribute:NSFontAttributeName value:[UIFont boldSystemFontOfSize:21.0] range:NSMakeRange(0, messageText.length)];
    [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithRed:0.230 green:0.777 blue:0.316 alpha:1.000] range:NSMakeRange(0, messageText.length)];
...
- (void)defaultSetup {
    ...
    self.cancelArrowText = [[NSMutableAttributedString alloc] initWithString:@"取消"];
    self.firstBtnText = [@"接受" uppercaseString];
    self.firstBtnTextColor = [UIColor whiteColor];
    self.secondBtnText = [@"取消" uppercaseString];
